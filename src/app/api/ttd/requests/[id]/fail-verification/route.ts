  ...
  try {
    ...
    if (!reason || reason.trim().length === 0) {
      return NextResponse.json(
        { error: "Validation failed: A failure reason is required." },
        { status: 400 }
      );
    }
    ...
    const updated = await prisma.$transaction(async (tx) => {
      ...
      await tx.activityLog.create({
        data: {
          ...
          action: "TTD_VERIFICATION_FAILED",
          details: `Verification failed for request ${record.requestNumber}. Reason: "${reason}"`,
          createdAt: new Date(),
        },
      });
    ...
    if (updated.createdById) {
      await sendNotification(
        updated.createdById,
        "TTD Request Verification Failed",
        `Verification failed for request ${updated.requestNumber} (${updated.applicantName}). Reason: ${reason}`,
        {
          type: "ttd",
    ...
    console.error("Error failing TTD verification:", err);
    return NextResponse.json(
